...
class ObstacleAvoidanceNavigation(Node):
    def __init__(self):
        ...
        self.declare_parameter('goal_x', 3.0)  # Goal position X
        self.declare_parameter('goal_y', 2.0)  # Goal position Y
        self.declare_parameter('goal_tolerance', 0.0)  # Distance tolerance to goal
    ...
    def lidar_callback(self, msg):
        """Process lidar data to detect obstacles - simplified and more direct"""
        self.obstacles = []
        obstacle_points = []
        ...
        try:
            for point in pc2.read_points(msg, field_names=('x', 'y', 'z'), skip_nans=True):
                ...
                # Filter by height - keep ground level obstacles
        ...
        if immediate_obstacles:
            ...
            if len(front_obstacles) > 0:
                ...
                if len(left_obstacles) < len(right_obstacles):
                    ...
                    cmd.linear.x = 0.0  # Move slowly while turning
            ...
            if abs(angle_diff) > math.radians(30):
                cmd.linear.x = self.max_linear_speed * 0.8  # Turn in place
            elif abs(angle_diff) > math.radians(10):
                cmd.linear.x = self.max_linear_speed * 0.8  # Slow forward
            else:
