...
class DWBObstacleAvoidance(Node):
    ...
    def cloud_callback(self, msg: PointCloud2):
        ...
        try:
            for point in pc2.read_points(msg, field_names=('x', 'y', 'z'), skip_nans=True):
                ...
                # Skip points too close (likely robot parts) or too far
                if distance < self.robot_radius * 1.5 or distance > self.detection_distance:
                    continue
                
                # More strict height filtering to remove ground/ceiling
                if z < 0.1 or z > 1.5:  # Only consider points between 10cm and 1.5m height
                    continue
                
                # Additional filtering for very low points that might be ground
                if z < 0.3 and distance > 3.0:  # Remove distant low points (likely ground)
                    continue
    ...
    def control_loop(self):
        ...
        # Throttled logging (manual implementation)
